...
const EXAMPLES_BY_ROLE: Record<string, string[]> = {
  "Group CFO": [
    ...
    "Show ROI trend of Project Orion over the last 3 years",
    "EBIT margin trend FY2023 to FY2025",
  ],
  "BU General Manager": [
  ...
  Analyst: [
    ...
    "Show me budget variance Q2 FY2024",
    "ROI trend for visible projects",
    "What is EBIT margin?",
  ],
};
  ...
  return (
    <div className="app">
      <aside className="left-pane">
        <div className="brand">Ask Finance</div>
        ...
        <header className="chat-header">
          <div>
            ...
            <div className="scope-badge">
              {activeUser ? `${activeUser.role} · ${activeUser.scope_description}` : "loading…"}
            </div>
          </div>
                ...
                <div className="tool-trace">
                  {m.trace.tool_calls.map((tc, j) => (
                    <div key={j}>
                      ...
                      {Object.keys(tc.input).length > 0 ? `(${JSON.stringify(tc.input)})` : "()"}
                    </div>
                  ))}
                </div>
              )}
